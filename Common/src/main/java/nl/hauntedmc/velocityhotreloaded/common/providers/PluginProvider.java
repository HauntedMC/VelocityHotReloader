...
public interface PluginProvider<P, D extends VHRPluginDescription> {

    default File getPluginsFolder() {
        return VHRApp.getPlugin().getDataFolder().getParentFile();
    }
    ...
    default List<P> getPluginsDependingOn(String pluginId) {
        ...
        for (P loadedPlugin : getPlugins()) {
            VHRPluginDescription description = getLoadedPluginDescription(loadedPlugin);
